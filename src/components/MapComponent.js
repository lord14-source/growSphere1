...
import { GoogleMap, Marker, MarkerF, OverlayView, PolygonF, useJsApiLoader } from '@react-google-maps/api';
...
const MapComponent = () => {
  ...
  const options = {
    mapTypeId: "hybrid", // Hybrid: Satellite with labels
    mapTypeControl: true,
    mapTypeControlOptions: isLoaded
      ? {
          style: window.google?.maps?.MapTypeControlStyle.DEFAULT, // Only access if window.google is defined
          position: window.google?.maps?.ControlPosition.TOP_LEFT, // Only access if window.google is defined
        }
      : null,
    zoomControl: true,
...
/>
{/* This is the t=code for the map Lines */}
